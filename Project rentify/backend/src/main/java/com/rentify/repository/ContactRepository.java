...
public interface ContactRepository extends JpaRepository<Contact, Long> {
    ...
    // Find contacts by user or email
    @Query("SELECT c FROM Contact c WHERE c.user = :user OR c.email = :email ORDER BY c.createdAt DESC")
    List<Contact> findByUserOrEmailOrderByCreatedAtDesc(com.rentify.entity.User user, String email);
}